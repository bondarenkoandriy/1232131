...
namespace Oxide.Plugins
{
    ...
    internal class BetterNpc : RustPlugin
    {
        ...
        internal class ControllerSpawnPoint : FacepunchBehaviour
        {
            ...
            {
                List<NpcWear> wearItems = config.WearItems ?? new List<NpcWear>();
                List<NpcBelt> beltItems = config.BeltItems ?? new List<NpcBelt>();
                foreach (NpcBelt belt in beltItems.Where(b => b != null))
                {
                    if (belt.Mods == null) belt.Mods = new List<string>();
                    if (belt.Ammo == null) belt.Ammo = string.Empty;
                }
                string name = (config.Names != null && config.Names.Count > 0) ? config.Names.GetRandom() : "NPC";
                string kit = (config.Kits != null && config.Kits.Count > 0) ? config.Kits.GetRandom() : string.Empty;
                ...
                if (beltItems.Any(x => x.ShortName == "rocket.launcher" || x.ShortName == "explosive.timed")) states.Add("RaidState");
                return new JObject
                {
                    ["Name"] = name,
                    ["WearItems"] = new JArray(wearItems.Select(x => new JObject { ["ShortName"] = x.ShortName, ["SkinID"] = x.SkinID })),
                    ["BeltItems"] = new JArray(beltItems.Select(x => new JObject { ["ShortName"] = x.ShortName, ["Amount"] = x.Amount, ["SkinID"] = x.SkinID, ["Mods"] = new JArray(x.Mods ?? new List<string>()), ["Ammo"] = x.Ammo ?? string.Empty })),
                    ["Kit"] = kit,
